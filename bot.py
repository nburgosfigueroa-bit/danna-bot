...
logger.info(f"TELEGRAM_TOKEN presente: {bool(TELEGRAM_TOKEN)}")
...
if not SUPABASE_URL:
    raise ValueError("SUPABASE_URL no encontrado en variables de entorno")
if not SUPABASE_KEY:
    raise ValueError("SUPABASE_ANON_KEY no encontrado en variables de entorno")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Usa el menú para crear solicitudes de trabajo.",
        parse_mode="Markdown",
...
async def ayuda(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📖 *Cómo usar DANNA:*\n\n"
        "1. Toca *Nueva Solicitud* para reportar un trabajo\n"
        "2. Selecciona el tipo de trabajo\n"
        "3. Indica el sector\n"
        "4. Describe el problema\n"
        "5. Adjunta una foto (opcional)\n\n"
        "Tu solicitud queda registrada automáticamente ✅",
        parse_mode="Markdown",
        reply_markup=MENU_PRINCIPAL
    )
...
async def recibir_descripcion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📸 ¿Tienes una foto? Envíala ahora.\n\nSi no tienes, escribe *sin foto* para continuar.",
        parse_mode="Markdown",
...
async def recibir_foto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        ...
        foto_url = f"telegram_file_{foto.file_id}"
    await guardar_solicitud(update, context, foto_url)
    return ConversationHandler.END
...
async def sin_foto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text.lower() == "❌ cancelar":
        await update.message.reply_text("Cancelado.", reply_markup=MENU_PRINCIPAL)
        return ConversationHandler.END
    await guardar_solicitud(update, context, None)
    return ConversationHandler.END

async def guardar_solicitud(update: Update, context: ContextTypes.DEFAULT_TYPE, foto_url):
    ...
    try:
        ...
        await update.message.reply_text(
            f"⚠️ OT generada: {ot_numero}\nError al guardar. Contacta al administrador.",
            reply_markup=MENU_PRINCIPAL
        )
...
async def mis_solicitudes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for s in resultado.data:
            estado_emoji = {"pendiente": "🟡", "en_proceso": "🔵", "completada": "✅"}.get(s["estado"], "⚪")
            fecha = s["fecha_creacion"][:10]
            texto += f"{estado_emoji} `{s['ot_numero']}`\n   {s['tipo_trabajo']} · {s['sector']} · {fecha}\n\n"
        ...
        await update.message.reply_text("Error al consultar. Intenta más tarde.", reply_markup=MENU_PRINCIPAL)
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex("^📍 Mi Sector$"), lambda u, c: u.message.reply_text("Función próximamente.", reply_markup=MENU_PRINCIPAL)))
    logger.info("🌿 DANNA Bot iniciando...")
    app.run_polling()
