...
async def guardar_solicitud(update: Update, context: ContextTypes.DEFAULT_TYPE, foto_url):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"Tu OT está en estado: *PENDIENTE*",
            parse_mode="Markdown",
            reply_markup=MENU_PRINCIPAL
        )
        ...
        await update.message.reply_text(
            f"⚠️ Solicitud generada (OT: {ot_numero}) pero hubo un error al guardar.\n"
...
async def mis_solicitudes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        texto = "📋 *Tus últimas 5 solicitudes:*\n\n"
        for s in resultado.data:
            ...
            texto += f"   {s['tipo_trabajo']} · {s['sector']} · {fecha}\n\n"
